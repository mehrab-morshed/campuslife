...
def main():
  loadDataForDays("campusdata/appforeground.csv", 0, 60)
...
def loadDataForDays(filepath, skipdays, numdays):
  ...
  column_names = ['timestamp', 'device_id', 'package', 'app', 'system_app']
  ...
  for i in range(0, numdays):
    ...
    df_dayi.to_csv('day' + 'm' + str(start_date.month) + 'd' + str(start_date.day) + '-app.csv', sep='\t', index=False)
