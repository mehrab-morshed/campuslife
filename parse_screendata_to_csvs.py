...
def main():
  loadDataForDays("campusdata/screen.csv", 0, 60)
...
def loadDataForDays(filepath, skipdays, numdays):
  ...
  column_names = ['timestamp', 'device_id', 'status']
  column_indexes = [1, 2, 3]
  ...
  for i in range(0, numdays):
    ...
    df_dayi.to_csv('day' + 'm' + str(start_date.month) + 'd' + str(start_date.day) + '-screen.csv', sep='\t', index=False)
